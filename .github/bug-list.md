...
| 外部 cron 接口在某些环境里可被未授权调用 | `/api/cron/reminders` 把“未配置 `CRON_SECRET`”当成合法状态，导致非 Vercel 请求默认放行 | `backend/src/routes/cron.ts` 必须要求显式配置 `CRON_SECRET`；未配置时返回 503，已配置但不匹配时返回 401 |
| 用户选择“不提醒”却仍在开始时间收到提醒 | `normalizeReminderCode()` 将未知值和 `none` 统一降级后，`syncReminders()` 没有跳过 `none`，仍写入 `pending` 记录 | `backend/src/services/reminders.service.ts` 中在生成 `remindAt` 前先过滤 `code === 'none'`，禁止把“无提醒”写入待发送队列 |
| 保存脑图时报错后，内容和提醒状态可能部分提交 | 导图更新与提醒重建不在同一事务内，失败时会出现脑图已更新但提醒被删空或未重建的半成功状态 | `backend/src/routes/maps.ts` 必须用同一 `db.transaction()` 包裹 mindmap 更新和 `syncReminders()`；服务层接收事务对象执行删除/插入 |
| OAuth 代理读取 JSON 后再转发原始响应会返回空 body 或抛 stream 已消费错误 | `frontend/src/app/auth/[...path]/route.ts` 直接 `await resp.json()` 后继续复用同一个 `resp.body` | 用 `resp.clone()` 读取 JSON；仅对白名单 Google OAuth URL 做 `redirect_uri` 重写，原始响应体继续透传 |
